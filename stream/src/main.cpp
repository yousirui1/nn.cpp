...
static bool stream_callback(const float *audio, uint32_t n_samples, void *user_data)
{
    LOG_DEBUG("n_samples %d", n_samples);
    ...
    fwrite(audio, sizeof(float), n_samples, fp);
    return true;
}

int cosyvoice_stream_inference(struct ggml_handle_t *ggml_handle, const char *text, void *param)
{
    struct cosyvoice_model_t *model = (struct cosyvoice_model_t *)ggml_handle->model;
    ...
    if (mode == "cross-lingual")
        ok = cosyvoice_tts_cross_lingual_stream(model->tts_ctx, text, speed, stream_callback, NULL);
    else if (mode == "zero-shot")
        ok = cosyvoice_tts_zero_shot_stream(model->tts_ctx, text, speed, stream_callback, NULL);
    else
        ok = cosyvoice_tts_instruct_stream(model->tts_ctx, text, instruction.c_str(), speed, stream_callback, NULL);
...
{
    struct ggml_handle_t ggml_handle = {0};
    const char *model_data = "/home/ysr/project/models/gguf/cosyvoice-gguf/CosyVoice3-2512_F32.gguf";

    load_cosyvoice_model(&ggml_handle, model_data, 0);
    cosyvoice_stream_inference(&ggml_handle, "欢迎光临欢迎光临欢迎光临欢迎光临欢迎光临欢迎光临", NULL);
