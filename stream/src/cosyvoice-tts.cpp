...
bool cosyvoice_model_3::token2wav_ext(const int* token_ids, uint32_t n_tokens, float speed, cosyvoice_prompt_t prompt, uint32_t* offset, bool streaming, bool finalize, cosyvoice_generated_speech_ptr result)
{
    ...
    // ================================================================
    // Phase 2: Flow decode steps (完全不变)
    // ================================================================
    ggml_tensor* t_leaf;
    ...
    // ================================================================
    // Phase 3: Build speech_feat
    //
    // 改动：不再把 flow_cache 拼到 vocoder 输入中。
    //       vocoder 只处理 feat（新帧），每帧独立输出。
    //       同时保留 flow_cache 保存，供调试或未来使用。
    // ================================================================
    ggml_reset(ctx1.get());

    ggml_tensor* speech_feat = ggml_new_tensor_2d(ctx1.get(), feat->type, feat->ne[0], feat->ne[1]);

    {
        auto buft = ggml_backend_get_default_buffer_type(backend.get());
        auto alloc_size = ggml_backend_buft_get_alloc_size(buft, speech_feat);

        if (!token2wav_buffer || alloc_size > ggml_backend_buffer_get_size(token2wav_buffer.get()))
        {
            auto new_buf = ggml_backend_alloc_ctx_tensors_from_buft(ctx1.get(), buft);
            GGML_ASSERT(new_buf != nullptr);
            reset_shared_buffer(std::move(new_buf));
        }
        else
        {
    ...
    // 保存 flow_cache（同步读取，避免数据竞争）
    if (!finalize)
    {
        const size_t old_size = worker->flow_cache.size();
        const size_t add_size = static_cast<size_t>(ggml_nbytes(feat));
        worker->flow_cache.resize(old_size + add_size);
        ggml_backend_tensor_get(feat,
            worker->flow_cache.data() + old_size, 0, add_size);
    }
...
{
    // ---- 调试信息 ----
    std::cout << "[DEBUG] flow_cache.size()=" << worker->flow_cache.size()
              << ", feat->ne[0]=" << feat->ne[0]
              << ", feat->ne[1]=" << feat->ne[1]
              << ", cache_length=" << cache_length
              << ", speech_feat shape=[" << speech_feat->ne[0] << ", " << speech_feat->ne[1] << "]"
              << std::endl;

    auto buft = ggml_backend_get_default_buffer_type(backend.get());
    auto alloc_size = ggml_backend_buft_get_alloc_size(buft, speech_feat);

    if (alloc_size > ggml_backend_buffer_get_size(token2wav_buffer.get())) {
        auto new_buf = ggml_backend_alloc_ctx_tensors_from_buft(ctx1.get(), buft);
        GGML_ASSERT(new_buf != nullptr);
        reset_shared_buffer(std::move(new_buf));
    } else {
    ...
    const size_t cache_bytes = speech_feat->nb[1] * cache_length;
    ggml_backend_tensor_set_async(backend.get(), speech_feat, worker->flow_cache.data(), 0, cache_bytes);

    ggml_tensor* dst_view = ggml_view_2d(ctx1.get(), speech_feat,
        speech_feat->ne[0], feat->ne[1],
        speech_feat->nb[1], speech_feat->nb[1] * cache_length);

    if (dst_view->buffer == nullptr)
        dst_view->buffer = speech_feat->buffer;

    ggml_backend_tensor_copy_async(backend.get(), backend.get(), feat, dst_view);
}
    else 
    {    
        ggml_backend_tensor_alloc(token2wav_buffer.get(), speech_feat,
...
if (cache_length != 0)
{
    ...
    if (alloc_size > ggml_backend_buffer_get_size(token2wav_buffer.get())) {
        auto new_buf = ggml_backend_alloc_ctx_tensors_from_buft(ctx1.get(), buft);
        GGML_ASSERT(new_buf != nullptr && "failed to allocate buffer for speech_feat");
        reset_shared_buffer(std::move(new_buf));
    } else {
        ggml_backend_tensor_alloc(token2wav_buffer.get(), speech_feat,
            ggml_backend_buffer_get_base(token2wav_buffer.get()));
    ...
    // ---- 检查点：确认 speech_feat 已成功分配到 buffer ----
    GGML_ASSERT(speech_feat->buffer != nullptr && "speech_feat has no buffer after allocation");

    const size_t cache_bytes = speech_feat->nb[1] * cache_length;
    ggml_backend_tensor_set_async(backend.get(), speech_feat, worker->flow_cache.data(), 0, cache_bytes);

    // ---- 关键修改：用 ctx1（而非 ctx0）创建 view，并显式继承 buffer ----
    ggml_tensor* dst_view = ggml_view_2d(ctx1.get(), speech_feat,
        speech_feat->ne[0], feat->ne[1],
        speech_feat->nb[1], speech_feat->nb[1] * cache_length);

    // 某些 ggml 版本的 ggml_view_2d 不会自动设置 result->buffer
    if (dst_view->buffer == nullptr)
        dst_view->buffer = speech_feat->buffer;
