...
matrix cosyvoice_frontend_context::extract_speech_feat(float* speech, uint32_t len)
{
    ...
    // Note: SVML intrinsics (_mm256_log_ps, etc.) require VS2019+ or Intel compiler.
    // Use scalar log for VS2017 compatibility.
    for (; mel_cur != mel_end; ++mel_cur)
    {
...
tokens_t cosyvoice_frontend_context::extract_speech_token(float* signal, uint32_t len)
{
    ...
    // Note: SVML intrinsics (_mm256_log10_ps, etc.) require VS2019+ or Intel compiler.
    // Use scalar log10 for VS2017 compatibility.
    ...
    for (auto i : info.GetShape())
        count *= i;
    auto tokens = std::shared_ptr<int>(new int[count], std::default_delete<int[]>());
...
matrix cosyvoice_frontend_context::extract_spk_embedding(float* speech, uint32_t len)
{
    ...
    std::unique_ptr<float[]> buffer(new float[padded_win_size]);
    // Zero-pad frame (win_size=400) to padded_win_size (512) for FFT
    std::unique_ptr<float[]> fft_input(new float[padded_win_size]);
...
cosyvoice_prompt_speech_t cosyvoice_frontend_context::frontend_vc_prompt_speech(float* speech, uint32_t len, uint32_t sample_rate)
{
    ...
    if (sample_rate != 24000)
        audio_resample(speech, len, sample_rate, &speech_ptr, &speech_len, 24000);

    matrix speech_feat = extract_speech_feat(speech_ptr, speech_len);
    if (speech != speech_ptr)
        free_audio_data(speech_ptr);

    if (sample_rate != 16000)
        audio_resample(speech, len, sample_rate, &speech_ptr, &speech_len, 16000);
    else
    {
        speech_ptr = speech;
        speech_len = len;
