    ...
    uint32_t worker_no)
{
    ...
    for (int i = 0; i != k; ++i)
    {
        ...
        if (nucleus_random <= 0.0f)
        {
            ...
            int repeat_count = 0;
            {
                size_t _span_count = std::min((size_t)sampling_params->win_size, (size_t)n_accepted_tokens);
                for (size_t _si = 0; _si < _span_count; ++_si) {
                    auto t = accepted_tokens[n_accepted_tokens - 1 - _si];
                    if (t == sampled_token)
                        ++repeat_count;
                }
            }
